            ...
            <Box>
                {tsMembers.map(member => {
                    ...
                    memberActions = memberActions.filter(action => {
                        ...
                        if (actionGroupFilter === 'moveToDone') return action.type === 'updateCard' && action.data?.listAfter?.name && action.data.listAfter.name.toLowerCase().includes('done');
                    ...
                    const coloredActionCounts = {
                        ...
                        moveToDone: memberActions.filter(a => a.type === 'updateCard' && a.data?.listAfter?.name && a.data.listAfter.name.toLowerCase().includes('done')).length,
                        ...
                        <Paper key={member.id} sx={{ mb: 3, p: 2, borderRadius: 3, boxShadow: 1 }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1 }}>
                                ...
                                <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                                    {coloredActionCounts.complete > 0 && (
                                        <Chip label={`Complete card: ${coloredActionCounts.complete}`} color="success" size="small" sx={{ fontWeight: 700, fontSize: 13 }} />
                                    )}
                                    {coloredActionCounts.moveToDone > 0 && (
                                        <Chip label={`Move to Done: ${coloredActionCounts.moveToDone}`} color="success" size="small" sx={{ fontWeight: 700, fontSize: 13 }} />
                                    )}
                                    {coloredActionCounts.moveToDoing > 0 && (
                                        <Chip label={`Move to Doing: ${coloredActionCounts.moveToDoing}`} color="warning" size="small" sx={{ fontWeight: 700, fontSize: 13 }} />
                                    )}
                                    {coloredActionCounts.moveToWaitingToFixFromDev > 0 && (
                                        <Chip label={`Move to Waiting to fix (from dev): ${coloredActionCounts.moveToWaitingToFixFromDev}`} color="secondary" size="small" sx={{ fontWeight: 700, fontSize: 13 }} />
                                    )}
                                    {coloredActionCounts.moveToWaitingToFix > 0 && (
                                        <Chip label={`Move to Waiting to fix: ${coloredActionCounts.moveToWaitingToFix}`} color="warning" size="small" sx={{ fontWeight: 700, fontSize: 13 }} />
                                    )}
                                    {coloredActionCounts.moveToUpdateWorkflowOrWaitingAccess > 0 && (
                                        <Chip label={`Move to Update workflow/Waiting for access: ${coloredActionCounts.moveToUpdateWorkflowOrWaitingAccess}`} color="default" size="small" sx={{ fontWeight: 700, fontSize: 13 }} />
                                    )}
                                    {coloredActionCounts.leftCard > 0 && (
                                        <Chip label={`Left card: ${coloredActionCounts.leftCard}`} color="error" size="small" sx={{ fontWeight: 700, fontSize: 13 }} />
                                    )}
                                    {coloredActionCounts.commentCard > 0 && (
                                        <Chip label={`Comment card: ${coloredActionCounts.commentCard}`} color="info" size="small" sx={{ fontWeight: 700, fontSize: 13 }} />
                                    )}
                            ...
                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                                {memberActions.map(action => {
                                    ...
                                    const isMoveToDone = action.type === 'updateCard' && action.data?.listAfter?.name && action.data.listAfter.name.toLowerCase().includes('done');
