...
export const postDevCards = async (cardData) => {
    try {
        console.log('Data before posting:', cardData);
        const response = await fetch(`${API_URL}/dev-cards`, {
            method: 'POST',
...
export const getDevResolutionTimes = async (startDate, endDate) => {
    try {
        ...
        const response = await fetch(`${API_URL}/dev-cards?${queryParams}`, {
            headers: getAuthHeaders()
        });
